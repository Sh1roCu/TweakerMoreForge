...
public abstract class RenderUtilsMixin {
    ...
    @ModifyExpressionValue(
            ...
            at = @At(
                    value = "INVOKE",
                    target = "Lfi/dy/masa/malilib/render/InventoryOverlay$Context;entity()Lnet/minecraft/world/entity/LivingEntity;",
                    remap = true
            ),
            remap = false
    )
    private static LivingEntity serverDataSyncer4InventoryOverlay_entity(LivingEntity entity) {
        if (TweakerMoreConfigs.SERVER_DATA_SYNCER.getBooleanValue()) {
            if (!Minecraft.getInstance().isSingleplayer()) {
