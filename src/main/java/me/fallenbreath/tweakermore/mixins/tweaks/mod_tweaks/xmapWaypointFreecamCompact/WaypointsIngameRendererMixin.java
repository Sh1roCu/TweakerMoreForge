...
public abstract class WaypointsIngameRendererMixin {
    ...
    @ModifyVariable(
            ...
            at = @At(
                    value = "INVOKE_ASSIGN",
                    target = "Lnet/minecraft/client/Minecraft;getCameraEntity()Lnet/minecraft/entity/Entity;",
                    remap = true
            ),
            require = 0,
            remap = false
    ...
    /**
     * {@link #adjustCameraEntityForFreecam1} may fail if xmap choose to directly access field {@link Minecraft#cameraEntity}
    ...
    private Entity adjustCameraEntityForFreecamImpl(Entity cameraEntity) {
        if (TweakerMoreConfigs.XMAP_WAYPOINT_FREECAM_COMPACT.getBooleanValue()) {
            Entity freecam = Minecraft.getInstance().cameraEntity;
            if (freecam != null) {
