...
public class PistorderRenderer implements TweakerMoreIRenderer, IClientTickHandler {
        ...
        if (player.getMainHandItem().isEmpty() && !player.isShiftKeyDown()) {
            ...
            if (block instanceof PistonBaseBlock) {
                ...
                if (!extended || ((PistonBlockAccessor) block).getIsSticky()) {
                    this.click(world, pos, blockState, blockState.getValue(PistonBaseBlock.FACING), extended ? PistonActionType.RETRACT : PistonActionType.PUSH);
    ...
    public void onRenderWorldLast(WorldRenderContextImpl context) {
        ...
        texts.stream().map(tr -> Pair.of(client.player.distanceToSqr(tr.getPos()), tr)).filter(p -> p.getFirst() <= maxDistance * maxDistance).sorted(Collections.reverseOrder(Comparator.comparingDouble(Pair::getFirst))).forEach(p -> p.getSecond().render());
    }
