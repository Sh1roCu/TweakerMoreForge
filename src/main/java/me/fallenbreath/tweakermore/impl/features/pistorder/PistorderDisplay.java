...
public class PistorderDisplay {
    ...
    private Direction getPistonFacing() {
        return this.blockState.getValue(PistonBaseBlock.FACING);
    }
        ...
        switch (this.displayMode) {
            ...
            case INDIRECT:
                ...
                if (!this.isStickyPiston()) {
                    simulatedPistonPos = simulatedPistonPos.offset(this.getPistonFacing().getUnitVec3i());
                }
                break;
            case DISABLED:
                break;
    ...
    private void analyze(Level world, BlockPos simulatedPistonPos, Direction pistonFacing, PistonActionType PistonActionType) {
        ...
        if (PistonActionType.isRetract()) {
            blockReplacer.add(simulatedPistonPos.offset(pistonFacing.getUnitVec3i()), air);  // simulated piston head
        }
    ...
    private boolean checkState(Level world) {
        ...
        BlockGetter chunk = world.getChunkSource().getChunkNow(this.pistonPos.getX() >> 4, this.pistonPos.getZ() >> 4);
